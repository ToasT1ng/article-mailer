...
class TestPipeline:
    ...
    async def test_run_full_pipeline(self, test_settings):
        ...
        with (
            ...
            patch.object(
                Pipeline, "_crawl_contents", new=AsyncMock(return_value=articles)
            ),
            ...
            mock_summarizer.screen = AsyncMock(return_value=articles)
            mock_summarizer.select_and_summarize = AsyncMock(return_value=summaries)
        ...
        mock_summarizer.screen.assert_called_once_with(articles, 3)
        mock_summarizer.select_and_summarize.assert_called_once_with(articles, 3)
    ...
    async def test_run_deduplicates_via_db(self, test_settings):
        ...
        with (
            ...
            patch(
                "src.pipeline.crawl_article_content",
